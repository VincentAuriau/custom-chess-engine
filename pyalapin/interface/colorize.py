...
def color_one_img(pil_img, background_color, border_color, inner_color):
	...
	img = [np.vectorize(r_dict.get)(img), 
	...
	background = np.dstack([np.ones((500, 500))*background_color[i] for i in range(3)]).astype("uint8")
...
if __name__ == "__main__":
	...
	for img_name in os.listdir("../gs_illustrations"):
		...
		down_img_bb = color_one_img(img, down_color_cell_b, border_color_b, inner_color_b)
		down_img_ww = color_one_img(img, down_color_cell_w, border_color_w, inner_color_w)
		down_img_wb = color_one_img(img, down_color_cell_w, border_color_b, inner_color_b)
		down_img_bw = color_one_img(img, down_color_cell_b, border_color_w, inner_color_w)

		name = img_name.split("_")[0]
		name = {"pawn": "P", "bishop": "B", "knight": "N", "rook": "R", "queen": "Q", "king": "K"}[name]
	...
	bg_w = Image.fromarray(np.dstack([np.ones((500, 500))*color_cell_w[i] for i in range(3)]).astype("uint8"))
	bg_w.save(os.path.join("../temp_images", "w.png"))
	bg_b = Image.fromarray(np.dstack([np.ones((500, 500))*color_cell_b[i] for i in range(3)]).astype("uint8"))
	bg_b.save(os.path.join("../temp_images", "b.png"))

	down_bg_w = Image.fromarray(np.dstack([np.ones((500, 500))*down_color_cell_w[i] for i in range(3)]).astype("uint8"))
	down_bg_w.save(os.path.join("../temp_images", "down_w.png"))
	down_bg_b = Image.fromarray(np.dstack([np.ones((500, 500))*down_color_cell_b[i] for i in range(3)]).astype("uint8"))
