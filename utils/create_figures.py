...
bg_w = Image.fromarray(
    np.dstack([np.ones((500, 500)) * color_cell_w[i] for i in range(3)]).astype("uint8")
)
bg_w.save(os.path.join("../temp_images", "w.png"))
bg_b = Image.fromarray(
    np.dstack([np.ones((500, 500)) * color_cell_b[i] for i in range(3)]).astype("uint8")
)
bg_b.save(os.path.join("../temp_images", "b.png"))

down_bg_w = Image.fromarray(
    np.dstack([np.ones((500, 500)) * down_color_cell_w[i] for i in range(3)]).astype(
        "uint8"
    )
)
down_bg_w.save(os.path.join("../temp_images", "down_w.png"))
down_bg_b = Image.fromarray(
    np.dstack([np.ones((500, 500)) * down_color_cell_b[i] for i in range(3)]).astype(
        "uint8"
    )
)
